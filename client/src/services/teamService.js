...
console.log("Şu anki token:", getToken());
...
export const listTeams = async ({page = 1, limit = 10, filterType = "all"} = {}) => {
  ...
  // 1. DÜZELTME: Eğer sunucudan hata dönerse (401, 404, 500 vb.) işlemi burada kes
  if (!res.ok) {
    // 401 ise kullanıcıyı logine yönlendirme mantığı da eklenebilir
    throw new Error(`Veriler çekilemedi. Sunucu Hatası: ${res.status}`);
  }
  
  const data = await res.json();

  // 2. DÜZELTME: Verinin güvenli bir şekilde map'lenmesi
  // Eğer data.teams yoksa, boş bir dizi [] döndürerek map'in çökmesini engelleriz
  const teamsArray = Array.isArray(data) ? data : (data.teams || []);

  return {
    ...data,
    teams: teamsArray.map(mapTeam),
  };
};
