...
const ProfileScreen = () => {
  ...
  const firstName = profile?.firstName || profile?.ad || (profile?.name ? profile.name.split(' ')[0] : '');
  const lastName = profile?.lastName || profile?.soyad || (profile?.name ? profile.name.split(' ').slice(1).join(' ') : '');
  const fullName = profile?.name || (firstName ? `${firstName} ${lastName}`.trim() : (profile?.username || "Kullanıcı"));
  const initialLetter = (profile?.name ? profile.name[0] : (firstName ? firstName[0] : (profile?.username ? profile.username[0] : "M"))).toUpperCase();
